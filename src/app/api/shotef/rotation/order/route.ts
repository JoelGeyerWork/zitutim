...
import { rotationOrderSchema } from "@/lib/rotation";
import { reorderShotefMembers } from "@/lib/shotef";
import {
  forbiddenResponse,
  getSessionFrom,
  isSameOrigin,
...
export async function PUT(request: Request) {
  ...
  try {
    const result = await reorderShotefMembers(parsed.data.ids);
    if (!result.ok) {
    ...
    console.error("PUT /api/shotef/rotation/order failed", error);
    return NextResponse.json(
